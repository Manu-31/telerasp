...
#-------------------------------------------------------------
def insertTeleinfoMySQL(dbCnx, ti) :
...
# est a False).
#-------------------------------------------------------------
def getLastTeleinfoFromDataBase(dbCnx) :
   result = {}
...
def processOneFrame(tr, dbCnx) :
   ...
   if (ti != None) :
      ...
      if (populateDataBase) :
         insertTeleinfoMySQL(dbCnx, ti) # ICI ?
      if ('frame' in debugFlags) :
...
def webGetLastTeleinfo() :
   ...
   if (populateDataBase) :
      ...
      # Finalement si on ne peut rien on va dans la base
         ...
         ti = getLastTeleinfoFromDataBase(database)
   return ti
...
# Connexion a la base de donnees. Cette connexion est
# necessaire meme si populateDataBase == False car on
# utilise la base en lecture.
